...
def readTreeStruct(jsonData,path,index,fileName):
    ...
    if (childs != None):
        childsList = {}
        for value in childs:
            if(value.get(idName) != None):
                objobj = jsonData[value[idName]];
                if(objobj ==None):
                    continue
                childsName = objobj.get(nameName)
                if(childsName != None):
                    if(childsList.get(childsName) != None):
                        newPath = path + "/" + childsName
                        fileDict = DICT.get(fileName);
                        if (fileDict == None):
                            DICT[fileName] = {}
                        DICT[fileName][newPath] = DEFAULT_STR;
                    else:
                        childsList[childsName] = value;
            readTreeStruct(jsonData, path, value[idName],fileName);
