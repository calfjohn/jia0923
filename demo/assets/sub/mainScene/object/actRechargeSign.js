...
cc.Class({
    ...
    clickPageItem:function (event) {
        ...
        if(data.isTouch){//点击事件，领取奖励
            ...
            var state = this.activityLogic.getRechargeSignState(data.clickIdx);
            if(state === constant.SignState.CANGET){//领取奖励
                this.activityLogic.req_Continue_Charge_Rec(1,data.clickIdx + 1,false);
            }
        }else{
    ...
    init (data) {
        this.signData = data;
        this.continueCharge = this.activityLogic.getContinueCharge();
        this.chargeReward = this.activityLogic.getChargeReward();
    ...
    refresh(){
        ...
        this.state = this.signData.userData.ContinueChargeReward[0];
        this.btn.interactable = this.state !== constant.SignState.GETED;
        if(this.state === constant.SignState.GETED){
            this.btnLabel.string = uiLang.getMessage(this.node.name,"received");
        }else if(this.state === constant.SignState.CANGET){
            this.btnLabel.string = uiLang.getMessage(this.node.name,"receive");
        }else{
            this.btnLabel.string = uiLang.getMessage(this.node.name,"go");
        }
        ...
        for (var i = 0 , len = this.continueCharge.length; i < len; i++) {
            var obj = this.signData.userData.ContinueChargeState[i];
    ...
    getEvent:function () {
        if(this.state === constant.SignState.CANGET){//领取奖励
            this.activityLogic.req_Continue_Charge_Rec(2,5,false);
        }else{
