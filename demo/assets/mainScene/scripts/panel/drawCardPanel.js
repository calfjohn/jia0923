...
cc.Class({
    ...
    openDrawEquip(){
        uiManager.closeUI(uiManager.UIID.DRAW_CARD);
        setTimeout(function () {
            uiManager.openUI(uiManager.UIID.DRAW_EQUIP);
        }.bind(this),50)
    },
    ...
    refeshDrawFree(){
        ...
        this.time = 0;
        this.freeBtn.interactable = data.DrawNormalFree > 0;
        if(data.DrawNormalFree > 0){
            var num = data.DrawNormalFreeDailyNum + data.DrawNormalFree;
            this.freeLabel.string = uiLang.getMessage(this.node.name,"free") +  num + uiLang.getMessage(this.node.name,"count");
        }else{
            this.time = data.DrawNormalFreeNext - this.timeLogic.now();
            this.freeLabel.string = this.timeLogic.getCommon1CoolTime(this.time);
        }
        if(this.time <= 0 && data.DrawNormalFreeNext === 0){
            this.freeLabel.string = uiLang.getMessage(this.node.name,"free") +  0 + uiLang.getMessage(this.node.name,"count");
        }
        this.btnCostNode.active = data.DrawHighFree === 0;
        this.btnFreeNode.active = data.DrawHighFree > 0;
        this.btnFreeLabel.string = uiLang.getMessage(this.node.name,"free") + data.DrawHighFree + uiLang.getMessage(this.node.name,"count");
    },
    update:function (dt) {
    ...
    open (isShowAct) {
        ...
        this.initShopDraw();
        this.scoreShopNode.active = this.userLogic.isScoreShopOpen();
    },

    initShopDraw() {
        this.shopData = this.shopLogic.getShopDataByType(constant.ShopType.NEWBOX);
    ...
    showDrawCardAnim(rewards) {
        ...
        var callback1 = function () {
            uiManager.openUI(uiManager.UIID.ACT_DRAWCARD_REWARD,constant.DrawCardSrcEnum.ShopDrawCard,rewards,[],callback,this.monQuality);
        };
    ...
    clickDraw(event, cusData) {
        ...
        var data = this.shopLogic.getFreeDrawData();
        if(!(cusData === "1" && data.DrawHighFree > 0) && curDiamond < price) {
