(function () {
    ...
    if (cc.sys.isNative) {
        var lastVersion = cc.sys.localStorage.getItem('lastVersion');//最新版本号
        var versionFild = jsb.fileUtils.getStringFromFile("version.manifest");
        if(lastVersion && versionFild){
            var oldVersion = JSON.parse(versionFild).version;//APK原始版本
            var vA = oldVersion.split('.');
            var vB = lastVersion.split('.');
            var re = false;
            try {
            ...
            if(re){//APK原始大版本大于缓存大版本
                var storagePath = ((jsb.fileUtils ? jsb.fileUtils.getWritablePath() : '/') + 'zCodeFile');
                try {
                    jsb.fileUtils.removeDirectory(storagePath + "_temp/");
                    jsb.fileUtils.removeDirectory(storagePath + "/");
                } catch (e) {
                    cc.error("clear oldHotFail",e);
                }
                cc.sys.localStorage.setItem('lastVersion',"");
